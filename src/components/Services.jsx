...
const ServiceItem = ({ image, name, description }) => {
  return (
    <div className="flex flex-col lg:flex-row gap-8 justify-evenly shadow-xl rounded-xl items-center lg:items-start md:w-3/4">
      <div className="flex flex-col items-start gap-6 mt-4">
        <h2 className="font-bold text-2xl self-center">{name}</h2>
        <hr className="w-3/4 self-center border-1 border-black" />
        <p className="w-full pb-6 mb-auto text-center text-lg lg:text-xl italic">
          {description}
        </p>
      </div>
      <img
        className="w-96 h-52 md:h-full mb-auto object-cover rounded-b-xl"
        src={image}
        alt=""
      />
    </div>
